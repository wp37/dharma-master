...
const LicenseGate: React.FC<Props> = ({ onActivated }) => {
  ...
  const qrUrl = `${BANK_INFO.qrBaseUrl}?amount=${selectedPlan.priceNum}&addInfo=DHARMA_${selectedPlan.plan.toUpperCase()}&accountName=${encodeURIComponent(BANK_INFO.accountHolder)}`;
    ...
    <div className="fixed inset-0 z-[9999] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 overflow-y-auto">
      <div className="w-full max-w-lg my-auto">
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-black text-white tracking-tight">TUAI DHARMA MASTER</h1>
          <p className="text-sm text-slate-500 mt-1">AI Buddhist Wisdom Suite — Phật Pháp & Trí Tuệ Giác Ngộ</p>
        </div>
          ...
          {!showPayment ? (
            <>
              ...
              <input value={key} onChange={e => setKey(e.target.value.toUpperCase())}
                ...
                className="w-full bg-black border border-white/10 rounded-xl p-4 text-sm text-white font-mono tracking-wider text-center outline-none focus:border-teal-500/50 placeholder-white/20 mb-3" />
              ...
              <button onClick={handleActivate} disabled={loading || !key.trim()}
                className="w-full py-3 bg-gradient-to-r from-teal-600 to-teal-500 hover:from-teal-500 hover:to-teal-400 text-white font-bold rounded-xl flex items-center justify-center gap-2 transition-all disabled:opacity-50 shadow-[0_0_20px_rgba(13,148,136,0.2)]">
              ...
              <div className="text-[10px] text-slate-500 font-bold uppercase mb-3 flex items-center gap-2">
                <i className="fa-solid fa-tags text-teal-400" /> BẢNG GIÁ LICENSE — DHARMA MASTER
              </div>
              <div className="grid grid-cols-2 gap-2 mb-4">
                {PLANS.map(p => {
                  const info = getPlanLabel(p.plan);
                  return (
                    <div key={p.plan} className="bg-black/50 border border-white/5 rounded-lg p-3 text-center hover:border-teal-500/20 transition-all">
            ...
            <>
              ...
              <div className="bg-black/50 border border-green-500/20 rounded-xl p-4 space-y-2">
                ...
                {[
                  ...
                  { label: 'Nội dung CK', value: `DHARMA_${selectedPlan.plan.toUpperCase()}`, icon: 'fa-comment' },
                ].map(item => (
              ...
              <div className="mt-4 p-3 bg-teal-900/10 border border-teal-500/20 rounded-lg text-[10px] text-teal-200/80 text-center">
